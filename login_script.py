...
def attempt_login(context: BrowserContext, email: str, password: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        # 增加页面加载时间
        try:
           page.wait_for_load_state("networkidle", timeout=10000) # 增加等待时间
        except TimeoutError:
            logging.warning("页面加载超时 (登录后)")
            pass  #  继续检测
        ...
        try:
            ...
            # 尝试检测登录成功
            try:
                page.wait_for_url("https://client.webhostmost.com/clientarea.php", timeout=5000)  #  检查 URL
                #  增加一个更具体的页面元素检测。  例如， "Your Hosting Plans"
                try:
                    page.locator("text=Your Hosting Plans").wait_for(timeout=5000)
                    logging.info("登录成功（重定向检测）!")
                    return True, "登录成功（重定向检测）!"  # 登录成功的常见情况
                except TimeoutError:
                     return False, "登录失败：无法检测登录状态（URL 和 'Your Hosting Plans'）"

            except TimeoutError:
                return False, "登录失败：无法检测登录状态（无法检测页面变化）"  # 更通用的失败信息。
